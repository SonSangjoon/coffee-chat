...
One local ImageGen-based diagram explains the complete product without requiring ontology vocabulary first:
...
Visual review covers GitHub-like light and dark surroundings at desktop and mobile content widths. The solid backgrounds must remain intentional on both. Review also confirms that every visual inherits the approved cover, that every explanatory diagram visibly comes from ImageGen rather than generic vector boxes, and that no generated glyph artifact can be mistaken for text.
